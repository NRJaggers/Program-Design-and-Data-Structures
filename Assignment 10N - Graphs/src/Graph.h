...
void PathPriorityQue::penque(PathInfo r) {
    ...
    //fill up the node
		newPtr -> pathDetails.dist = r.dist;

		for (int i = 0; i < 20; i++)
		  newPtr -> pathDetails.path[i] = r.path[i];

		newPtr -> pathDetails.pathlen = r.pathlen;
    ...
    //case: head insertion
		else if (r.dist < head -> pathDetails.dist) {
			newPtr -> link = head;
			head = newPtr;
		}
    //case: mid and end insertion:
			...
			for (prev = head, cur = head -> link; cur != NULL; prev = prev -> link, cur = cur -> link) {
				if (newPtr -> pathDetails.dist < cur -> pathDetails.dist)
					break;
			}
...
void PathPriorityQue::dequeue(PathInfo & rec) {
	if (!isEmpty()){
		...
		rec.dist = head -> pathDetails.dist;
		for (int i = 0; i < 20; i++) {
			rec.path[i] = head -> pathDetails.path[i];
		}
		rec.pathlen = head -> pathDetails.pathlen;
		//Remove the node
		head = head -> link;
...
//================
	...
	bool Graph :: initializeGraph(string path){
	    ...
	    while (vertNum >= 0) {
	    	...
	    	vertName.shrink_to_fit();

	        //cout << vertNum << "\t" << vertName << endl ;//trouble shooting
	...
	void Graph :: MinPath(int from, int to){
		...
		//declarations
		  PathPriorityQue pathQueue;  	//ItemQue is a priority queue that queues ItemType items.
		  queue<int> adjVertQ;  //A regular queue that queues ints that is used to store adjacent vertices.
		  PathInfo pathDetails;
...
//			  marked[i] = 0;
		  ...
		  pathQueue.penque(pathDetails);
		  //start the deque/enque loop
		  while (!(pathQueue.isEmpty())) {
			  //Deque the item.
			  pathQueue.dequeue(pathDetails);
			  //break if target is reached.
			  if (pathDetails.path[pathDetails.pathlen - 1] == to)
				  break;
			  ...
			  p_dist = pathDetails.dist;
			  p_vertex = pathDetails.path[pathDetails.pathlen - 1];
			  p_pathlen = pathDetails.pathlen;
			  //if the item is not yet marked. find the next level items.
			  if (marked[pathDetails.path[pathDetails.pathlen - 1]] == 0) {
				  //mark the item
				  marked[pathDetails.path[pathDetails.pathlen - 1]] = 1;
				  ...
				  getAdjacent(pathDetails.path[pathDetails.pathlen - 1], adjVertQ);
				  ...
				  while (!adjVertQ.empty()) {
					  ...
					  if (marked[adjVert] == 0) {
						  ...
						  pathDetails.dist = p_dist + edges[p_vertex][adjVert];
						  //add the vertex to the path list in the item
						  pathDetails.path[p_pathlen] = adjVert;
						  //update the length of the used verix array list
						  pathDetails.pathlen = p_pathlen + 1;
		  ...
		  cout << "Minimum distance from " << vertices[from] << " to " << vertices[to] << " is " << pathDetails.dist << endl;
		  cout << "path:" << endl;
		  for (int i = 0; i < pathDetails.pathlen; i++) {
			  cout << vertices[pathDetails.path[i]] << endl;
		  }
